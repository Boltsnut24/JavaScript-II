...
// Problem 1
// Up each person's donation by $20.00 and see what the total is now.
let increasedDonation = runners.reduce(function(total, runners){
  return total = total + runners.donation + 20;
}, 0);

console.log(increasedDonation);
// Problem 2
// Fix the format of how donation is stored to real dollar symbols
let formatted = runners.map((runners) => {
  return runners.donation = '$' + runners.donation + '.00';
});
console.log(formatted);

// Problem 3
//Only the first 30 runners can compete now
let first30 = runners.filter((runners) => {
  return runners.id <= 30;
});
console.log(first30);